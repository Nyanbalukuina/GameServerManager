...
import gameservermanager.shared.construction.ConstructionProgressStatus
import gameservermanager.shared.construction.ConstructionProgressStepStatus
...
class ConstructPalworldServerFirewallTests {
    ...
    fun `Firewall設定後に起動失敗した場合は規則を解除する`() {
        ...
        val progress = dependencies.progressTracker.get(ConstructPalworldServer.GAME)
        assertThat(progress?.status).isEqualTo(ConstructionProgressStatus.ERROR)
        assertThat(progress?.steps?.first { it.id == "start" }?.status)
            .isEqualTo(ConstructionProgressStepStatus.ERROR)
    }
    ...
    private fun dependencies(): Dependencies {
        ...
        val progressTracker = ConstructionProgressTracker()
