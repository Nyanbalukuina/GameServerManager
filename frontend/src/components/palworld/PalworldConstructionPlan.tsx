...
export function PalworldConstructionPlan({ plan, preflight, isConstructing, constructionKind, constructionReport, realReport, constructionError, onRunDemoConstruction, onRunRealConstruction, onReturnToForm, demoEnabled, serverPassword, adminPassword, worldSettings }: Props) {
  ...
  return <>
    ...
    <section className="card">
      <div className="section-heading"><div><p className="section-number">02</p><h2>ゲーム設定</h2></div><p>PalWorldSettings.iniと自動運転へ保存する内容です。</p></div>
      <div className="settings-preview-grid">
        <section><h3>PalWorldSettings.ini</h3><dl>
        ...
        <section><h3>自動運転</h3><dl><dt>状態</dt><dd>{plan.automationEnabled ? '有効' : '無効'}</dd>{plan.automationEnabled && <><dt>停止時刻</dt><dd>{plan.shutdownTime}</dd><dt>起動時刻</dt><dd>{plan.startupTime}</dd></>}</dl></section>
      </div>
    </section>

    <section className="card"><div className="section-heading"><div><p className="section-number">03</p><h2>接続設定</h2></div></div><dl><dt>ゲームポート</dt><dd>{plan.gamePort}</dd><dt>RCONポート</dt><dd>{plan.rconPort}</dd><dt>接続元</dt><dd>{formatGamePortAccess(plan)}</dd></dl></section>
    <section className="card"><div className="section-heading"><div><p className="section-number">04</p><h2>パスワード</h2></div><p>PalWorldSettings.iniへ保存する内容です。</p></div><dl><dt>サーバーパスワード</dt><dd>{serverPassword || '未設定'}</dd><dt>管理者パスワード</dt><dd>{adminPassword || '未設定'}</dd></dl></section>
    ...
    <section className="card action-card">
      <div className="section-heading"><div><p className="section-number">06</p><h2>サーバー構築</h2></div><p>{kind === 'DEMO' ? '実環境を変更せず、設定と操作の流れを確認します。' : 'Palworldサーバーをインストールし、設定・Firewall・起動まで実行します。'}</p></div>
